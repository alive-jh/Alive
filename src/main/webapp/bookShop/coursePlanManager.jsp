...
function updateUI(){
	...
	for(var i=0;i<jsonData.length;i++){
			...
			newcontent = newcontent + '<td nowrap="nowrap" align="left" >'+jsonData[i].projectName+'</td>';
			newcontent = newcontent + '<td nowrap="nowrap" align="left" >'+jsonData[i].planType+'</td>';
...
function deleteData(index){
	if(confirm('你确定要删除数据吗？'))
	{
		...
		data:dataPar,
		dataType: "json",
		url: "<%=request.getContextPath() %>/bookApi/deleteCourseProjectSystem", 
...
function editData(index)
{
	...
	if(index.length == '0'){
		...
		$("#index").val(index);
		$("#id").val(myData.id);
		$("#projectName").val(myData.projectName);
		$("#planType").val(myData.planType);
		$("#lessonList").val(myData.lessonList);
		$("#sort").val(myData.sort);
	}
}
...
function updateData(index) {
     ...
     $.ajax({
          url: '<%=request.getContextPath() %>/bookApi/saveCourseProjectSystem' ,
          type: 'POST',
          data: formData,
          async: false,
          cache: false,
	...
	<div class="page-content">
		...
		<div class="page-header col-xs-12" style="float: left;width:100%">
			<form class="form-inline" role="form" method="post" name="searchForm"
				action="<%=request.getContextPath()%>/bookShop/coursePlanManager"
				...
				<div class="row">
					<div class="col-xs-12">
						<div class="table-responsive" style="text-align:right;">
							...
							<f:page page="${resultPage}" url="./coursePlanManager"
		...
		<div class="modal-dialog" style="width:900px;height: 1200px;">
			<div class="modal-content">
				...
				<div class="modal-body">
							...
							<div class="input-medium">
								<div class="input-group" style="width:500px;">
									<input class="form-control" name="projectName"
										id="projectName" type="text" data-rule="系统课程计划名称:required;"
					...
					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">课程ID集合（逗号分隔）</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width:500px;">
									<input class="form-control" name="lessonList"
										id="lessonList" type="text" data-rule="课程ID集合:required;"
										style="max-width:650px;width:100%;" />
								</div>
							</div>
						</div>
					...
					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">系统课程计划类型</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width:500px;">
									<input class="form-control" name="planType"
										id="planType" type="text" data-rule="系统课程计划类型:required;"
										style="max-width:650px;width:100%;" />
								</div>
							</div>
						</div>
					...
					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">系统课程计划排序</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width:500px;">
									<input class="form-control" name="sort"
										id="sort" type="text" data-rule="系统课程计划排序:required;"
										style="max-width:650px;width:100%;" />
								</div>
							</div>
						</div>
