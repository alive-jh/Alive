...
function updateUI(){
	...
	for(var i=0;i<jsonData.length;i++){
			var staStr=jsonData[i].status==1?"有效":"失效";
			var staCss=jsonData[i].status==1?"success":"danger";
			newcontent = newcontent + '<tr>';	
			newcontent = newcontent + '<td nowrap="nowrap" align="left" >'+jsonData[i].id+'</td>';
			newcontent = newcontent + '<td nowrap="nowrap" align="left" ><img src="'+jsonData[i].qrcodeUrl+'" style="width:40px;"/></td>';
			newcontent = newcontent + '<td nowrap="nowrap" align="left" >'+jsonData[i].planName+'</td>';
			newcontent = newcontent + '<td nowrap="nowrap" align="left" >'+jsonData[i].activeCount+'</td>';
...
function deleteData(index){
	if(confirm('你确定要删除数据吗？'))
	{
		...
		data:dataPar,
		dataType: "json",
		url: "<%=request.getContextPath() %>/bookApi/deleteCourseProjectActive", 
...
function editData(index)
{
	...
	if(index.length == '0'){
		...
		for ( var i = 0; i < systemProjects.length; i++) {
			$("#planId").append('<option value="'+systemProjects[i].id+'" >'+systemProjects[i].projectName+'</option>');
		}
	}else{
		//第一步，传输数组数据到弹出框
		...
		$("#index").val(index);
		$("#id").val(myData.id);
		$("#activeCount").val(myData.activeCount);
		$("#planId option").remove();
		for ( var i = 0; i < systemProjects.length; i++) {
			if(systemProjects[i].id==myData.planId){
				$("#planId").append('<option value="'+systemProjects[i].id+'" selected="selected" >'+systemProjects[i].projectName+'</option>');
			}else{
				$("#planId").append('<option value="'+systemProjects[i].id+'" >'+systemProjects[i].projectName+'</option>');
			}
		}
		...
		if(myData.status==1){
			$("#status").append("<option value='1' selected='selected' >有效</option>");
			$("#status").append("<option value='0' >失效</option>");
		}else{
			$("#status").append("<option value='0' selected='selected' >失效</option>");
			$("#status").append("<option value='1' >有效</option>");
		}
	}
}
...
function updateData(index) {
	 ...
	 var produceCount=$("#produceCount").val();
     var formData = new FormData($("#modifyData")[0]);
     $.ajax({
          url: '<%=request.getContextPath() %>/bookApi/saveCourseProjectActive' ,
          type: 'POST',
          data: formData,
          async: false,
          cache: false,
          ...
          success: function (returndata) {
			  ...
			  if(index.length == '0'){
					...
					//for(var z=0;z < activeProjects.length;z++){
						//alert(activeProjects[z].id);
					//}
					//刷新页面
					window.location.reload(true);
					//jsonData.push(result.data[z]); 
				    //updateTotalCount(1);
					
				}else{
					//更新数组数据
					jsonData[index]=result.data[0];
			  	}
			  //更新界面
			  //updateUI();
          },
...
<div class="main-content">
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb">
			...
			<li class="active">系统课程计划激活二维码管理</li>
		</ul>
		<!-- .breadcrumb -->
	...
	<div class="page-content">
		...
		<div class="page-header col-xs-12" style="float: left;width:100%">
			<form class="form-inline" role="form" method="post" name="searchForm"
				action="<%=request.getContextPath()%>/bookShop/coursePlanActive"
				...
				<div class="row">
					<div class="col-xs-12">
						<div class="table-responsive" style="text-align:right;">
							<table id="dataListTab"
								...
								<thead>
									<tr>
										<th width="100">ID</th>
									    <th width="100">二维码</th>
										<th width="100">系统课程计划名称</th>
										<th width="100">剩余激活次数</th>
										<th width="100">二维码状态</th>
							...
							<f:page page="${resultPage}" url="./coursePlanActive"
		...
		<div class="modal-dialog" style="width:900px;height: 1200px;">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">添加/修改激活二维码信息</h4>
				</div>
				<div class="modal-body">
				

					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">系统课程计划名称</label>
						<div class="col-sm-9">
					...
					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">生产激活码个数</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width:500px;">
									<input class="form-control" name="produceCount"
										id="produceCount" type="text" data-rule="生产激活码个数:required;"
					...
					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">激活次数</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width:500px;">
									<input class="form-control" name="activeCount"
										id="activeCount" type="text" data-rule="激活次数:required;"
										style="max-width:650px;width:100%;" />
								</div>
							</div>
						</div>
					...
					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">二维码状态</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width:500px;">
