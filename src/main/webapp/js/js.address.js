...
{
    var $cmbProvinceS = $('#'+_cmbProvince);
    var $cmbCityS = $('#'+_cmbCity);
    var $cmbAreaS = $('#'+_cmbArea);
	...
	//省
    var swiper1 = new Swiper('.data-list1', {
        direction : 'vertical',
        slidesPerView : 9,
        centeredSlides : true,
        ...
        onSlideChangeEnd: function(swiper1){
            pCIndex=swiper1.activeIndex;
           	swi2(swiper2,pCIndex,cCIndex,aCIndex);
            swiper2.activeIndex=0;
            cCIndex=0;
           	swi3(swiper3,pCIndex,cCIndex,aCIndex);
            swiper3.activeIndex=0;
            addrStr[2]=$('#data-box3 li.swiper-slide-active').html();
            addrStr[1]=$('#data-box2 li.swiper-slide-active').html();
            addrStr[0]=$('#data-box1 li.swiper-slide-active').html();
            
            $('#addrShow').val(addrStr.join(' '));
            $('#addrShow').attr('value',addrStr.join(' '));
    ...
    //市
    var swiper2 = new Swiper('.data-list2', {
        direction: 'vertical',
        slidesPerView: 9,
        centeredSlides: true,
        mousewheelControl: true,
    ...
    //区
    var swiper3 = new Swiper('.data-list3', {
        direction: 'vertical',
        slidesPerView: 9,
        centeredSlides: true,
        mousewheelControl: true,
	...
	//初始化
	swiper1['params'].onInit=swiInit(swi1,swiper1,$cmbProvinceS,defaultProvince);
	swiper2['params'].onInit=swiInit(swi2,swiper2,$cmbCityS,defaultCity);
	swiper3['params'].onInit=swiInit(swi3,swiper3,$cmbAreaS,defaultArea);

	function swiInit(swi,swiper,cmbElemtent,defaultValue){
		swi(swiper,pCIndex,cCIndex,aCIndex);
		cmbSelect(cmbElemtent,defaultValue,swiper,pCIndex);
        addrStr[2]=$('#data-box3 li.swiper-slide-active').html();
        addrStr[1]=$('#data-box2 li.swiper-slide-active').html();
        addrStr[0]=$('#data-box1 li.swiper-slide-active').html();
        $('#addrShow').val(addrStr.join(' '));
    ...
    function swi1(swiper,pCIndex,cCIndex,aCIndex){
        pS=[];
        var str= '<li class="swiper-slide">请选择省</li>';
        $cmbProvinceS.html('');
        for(var i=0; i<provinceList.length; i++)
        {
            cmbAddOption($cmbProvinceS,pS, provinceList[i].name, provinceList[i],i,swiper);
        }
    }

    function swi2(swiper,pCIndex,cCIndex,aCIndex){
        cS=[];
        var str= '<li class="swiper-slide">请选择市</li>';
        $cmbCityS.html('');
        for(var i=0; i< provinceList[pCIndex].cityList.length; i++)
        {
            cmbAddOption($cmbCityS,cS, provinceList[pCIndex].cityList[i].name, provinceList[pCIndex].cityList[i],i,swiper);
        }
    }

    function swi3(swiper,pCIndex,cCIndex,aCIndex){
        aS=[];
        var str= '<li class="swiper-slide">请选择区</li>';
        $cmbAreaS.html('');
        for(var i=0; i< provinceList[pCIndex].cityList[cCIndex].areaList.length; i++)
        {
            cmbAddOption($cmbAreaS,aS,provinceList[pCIndex].cityList[cCIndex].areaList[i],null,i,swiper);
        }
    }

	function cmbSelect(cmb, str,swiper,cIndex)
	{
		for(var i=1; i<cmb.find('li').length; i++)
		{
			if(cmb.find('li').eq(i).html() == str)
			{
                cIndex = i;
                swiper.slideTo(i,0);
			}
	...
	function cmbAddOption(cmb,s, str, obj,index,swiper)
	{
        s=[];
        s[index]='<li class="swiper-slide" data="'+str+'">'+str+'</li>';

        swiper.appendSlide(s);
        cmb.find('li')[index].obj = obj;
...
{name:'澳门', cityList:[{name:'香港',areaList:['香港']}
]}
];