...
$(function() {
    ...
    $("#file_upload").uploadify({
        ...
        'uploader'      : '<%=request.getContextPath()%>/UploadifySerlet',
        'auto'          : false,
        'fileTypeExts'  : '*.RAR',
	...
	$.ajax({
		...
		success: function(data){
			...
			for(var i=0;i<data.infoList.length;i++){
				...
				newcontent = newcontent + '<td nowrap="nowrap" align="left" ><button class="btn btn-xs btn-info"  onclick="editArticle('+i+','+data.infoList[i].mp3Type+')" data-toggle="modal" data-target="#mymodal-data"><i class="icon-edit bigger-120"></i></button><button onclick="deleteArticle('+data.infoList[i].id+')" class="btn btn-xs btn-danger"><i class="icon-trash bigger-120"></i></button></td>';
				newcontent = newcontent + '</td>';
...
$('#mymodal-data3').modal('hide');
				...
				if(type ==3)
				{
					...
					$("#kecheng").hide();
					$("#showPirce").hide();
					$('#classRoomPackage').show();
					
				}
				

				document.forms['addarticle'].mp3Type.value = type;
				...
				if(index.length == '0' )
				{
					...
					$('.tempFiles').find('.tag-info').not('.tag-modal').remove();
				...
				else
				{
					...
					{											
						document.getElementById('preview1').innerHTML = '<img width="80" height="80" src="'+data.infoList[index].logo1+'"/>';
					...
					{
						document.getElementById('preview2').innerHTML = '<img width="80" height="80" src="'+data.infoList[index].logo2+'"/>';
					...
					{
						document.getElementById('preview3').innerHTML = '<img width="80" height="80" src="'+data.infoList[index].logo3+'"/>';
			...
			$('.tempFiles').find('.tag-info').not('.tag-modal').remove();
			if (data.infoList[index].files != '' && data.infoList[index].files != undefined) {

				//document.forms['addarticle'].fileNames.value = data.infoList[index].files;

				var tempStr = data.infoList[index].files.split(',');

				for (var i = 0; i < tempStr.length; i++) {

					var ss = tempStr[i].split('>');
					addFiles(ss[1]);
	...
	jQuery(function($) {
		...
		$('.tag .close').click(function() {
			...
			var tempIds = document.forms['addarticle'].fileNames.value;
			tempIds = tempIds.replace($(this).parent().find('span').html() + ",", "");
			tempIds = tempIds.replace($(this).parent().find('span').html(), "");
			//document.forms['addarticle'].fileNames.value = tempIds;
	...
	function setItems() {
		...
		var temp = document.forms['addarticle'].labelId.value;
		if (temp != '') {
	...
	function addFile(info) {
		...
		var newTag = $('.tag-modal').clone(true);
		$(newTag).find('span').html(_html);
		$(newTag).appendTo($('.tempFiles')).css('display', '').removeClass('tag-modal');

		document.forms['addarticle'].fileNames.value = document.forms['addarticle'].fileNames.value + info + ",";
			...
			<form class="form-inline" role="form" method="post" name="searchForm" action="<%=request.getContextPath()%>/mall/mallManager" style="float: left; width: 100%">
				<table class="filterTable">
					...
					<tr>
						<td>名称</td>
						<td>
							<input type="text" name="name" value="${queryDto.name}">
						</td>
				...
				<div class="btn-group col-md-12">
					<button class="btn  btn-purple" type="button" data-toggle="modal" data-target="#mymodal-data3" onclick="editArticle('')">
...
<form class="article" method="post" name="addarticle" enctype="multipart/form-data" action="<%=request.getContextPath()%>/mall/saveMall">
	...
	<input type="hidden" name="mp3Type">
		...
		<div class="modal-dialog" style="width: 1100px; height: 1000px;">
			<div class="modal-content">
				...
				<div class="modal-body">
						...
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width: 910px;">
									...
									<div style="display: inline; position: relative;">
										<button class="btn btn-info btn-sm">上传图片</button>
										<input type="file" name="file2" id="file2" onchange="previewImage(this,'2');" style="opacity: 0; position: absolute; max-width: 80px; min-height: 30px; top: 0; bottom: 0; left: 0; right: 0;" />
									</div>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<div style="display: inline; position: relative;">
										<button class="btn btn-info btn-sm">上传图片</button>
									...
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<div style="display: inline; position: relative;">
										<button class="btn btn-info btn-sm" onclick="showAudios()">查看音频</button>
									</div>
							...
							<div class="input-medium" style="width: 910px;">
								<div class="input-group" style="width: 910px;">
									...
									<div style="position: relative; display: inline-block; margin: 15px;" onmouseover="$(this).find('input').css('display','');" onmouseout="$(this).find('input').css('display','none');">
										...
										<div id="preview1">
											<img src="<%=request.getContextPath()%>/images/picture.png" style="width: 50px; height: 50px;" />
										</div>
					...
					<div id="showMp3" class="form-group" style="height: 235px;">
					...
					<div id="kecheng" class="form-group" style="height: 35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">课程文件</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width: 910px;">
									...
									<div style="display: inline; position: relative;">
										...
										<input type="file" name="uploadify" id="file_upload" style="opacity: 0; position: absolute; max-width: 80px; min-height: 30px; top: 0; bottom: 0; left: 0; right: 0;" />
									</div>
									<div class="tempFiles tags" style="border: none; width: auto; padding-left: 0; width: 650px;">
									...
									<br />
									<button type="button" onclick="javascript:$('#file_upload').uploadify('upload','*')" class="btn btn-purple ml10 mb10">开始上传</button>
					...
					<div class="form-group" style="height: 335px;">
						<label class="col-sm-2 control-label no-padding-right text-right">描述</label>
						<div class="col-sm-10">
							<div class="input-medium">
								<div class="input-group">
					...
					<div id="showGuige" class="form-group" style="height: 200px;">
						<label class="col-sm-2 control-label no-padding-right text-right">
							<span id="name4">商品规格</span>
						</label>
						<div class="col-sm-10">
							<div class="input-medium">
								<div class="input-group">
									<button class="btn btn-info btn-sm" type="button" onclick="addmallspecifications()">
										<span id="name5">添加规格</span>
									</button>
									<table id="mallTab" style="width: 650px;" class="table table-striped table-bordered table-hover" style="text-align:left;">
										<thead>
											<tr>
												<th width="100">
													<span id="name6">规格</span>
												</th>
												<th width="100">
													<span id="name7">价格</span>
												</th>
												<th width="100">
													<span id="name8">库存</span>
												</th>
												<th width="120">操作</th>
											</tr>
										</thead>
					...
					<div id="showLabel" class="form-group" style="height: 50px;">
						...
						<div class="col-sm-10">
							<div class="input-medium">
								<div class="input-group">
									<button class="btn btn-primary" type="button" data-toggle="modal" data-target="#mymodal-data2">选择标签</button>
									<input type="hidden" name="item" id="uItems" />
								</div>
							</div>

							<div class="tags tagSelected" style="border: none; width: auto; padding-left: 0; width: 650px;"></div>
						...
						<div class="col-sm-10">
							<div class="input-medium">
								<div class="input-group" style="max-width: 250px; width: 100%;">
									...
									<button type="button" onclick="addKeyword()" placeholder="请输关键词,多个以,号分隔" class="btn btn-purple ml10 mb10">
										<span class="icon-plus">添加 
									</button>
					...
					<div id="showPirce" class="form-group" style="height: 35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">课程价格</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width: 910px;">
									...
									<div style="display: inline; position: relative;">
										...
										<input type="text" name="tempPrice" />
									</div>
					...
					<div id="classRoomPackage" class="form-group" style="height: 35px;display: none">
						<label class="col-sm-2 control-label no-padding-right text-right">
							<span id="name1"></span>
							请输入课堂包ID
						</label>
						<div class="col-sm-9">
							<div class="input-medium">
								<div class="input-group" style="width: 710px;">
									<input class="form-control" name="classRoomPackageId" id="classRoomPackageId" type="text" data-rule="课堂包ID" style="max-width: 650px; width: 100%;" />
								</div>
							</div>
						</div>
					</div>
	...
	<div class="modal-dialog" style="width: 400px;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" data-toggle="modal" data-target="#mymodal-data" onclick="editArticle('','3')" class="btn btn-primary">课程包</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
