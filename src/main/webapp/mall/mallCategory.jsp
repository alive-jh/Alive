	...
	$.ajax({
		...
		success: function(data){
			...
			for(var i=0;i<data.infoList.length;i++){
				...
				newcontent = newcontent + '<tr><td nowrap="nowrap" align="left" ><a href="<%=request.getContextPath()%>/mall/mallManager?catId='+data.infoList[i].cat_id+'">'+data.infoList[i].mark+data.infoList[i].cat_name+'</a></td>';
				...
				newcontent = newcontent + '<td nowrap="nowrap" align="left" ><button class="btn btn-xs btn-info"  onclick="showMallProduct('+data.infoList[i].cat_id+')" data-toggle="modal" data-target="#mymodal-data11">查看详情</button></td>';
...
function showMallProduct(catId)
{
	...
	$.ajax({
		...
		data:"",
		dataType: "json",
		url: "<%=request.getContextPath() %>/mall/searchMallProductByCatId?catId="+catId, 
		context: document.body, 
		beforeSend:function(){
			//这里是开始执行方法
			
			var mallTable = $('#mallTable'); 
			$("#mallTable tr:not(:has(th))").remove();
			
			mallTable.append("<tr><td colspan=\"20\" style=\"text-align: center;\"><img src=\"<%=request.getContextPath() %>/css/images/loading.gif\" />正在加载中，请等待...</td></tr>");
		...
		success: function(mallData){

			
			//数据执行完成
			var mallTable = $('#mallTable');
			$("#mallTable tr:not(:has(th))").remove();
			for(var i=0;i<mallData.infoList.length;i++){
				
				newcontent = newcontent + '<tr><input type="hidden" name="tempIds" value="'+mallData.infoList[i].id+'">';
				newcontent = newcontent + '<td nowrap="nowrap" align="left" >'+mallData.infoList[i].name+'</td>';
				newcontent = newcontent + '<td nowrap="nowrap" align="left" ><input type="text" name="tempSort" value="'+mallData.infoList[i].accountId+'"></td>';
...
function deleteArticle(id){
	...
	{
		document.forms['addarticle'].action ="deleteMallCate?cat_id="+id;
...
{
	document.forms['addarticle'].action ="saveMallCate";
	if(index.length=='0'){
	//增加
				...
				<div class="modal-dialog" style="width:1100px;">
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">添加/修改商品分类</h4>
						</div>
						...
						<div  id="divAccount" class="form-group" style="height:35px;">
							<label class="col-sm-2 control-label no-padding-right text-right">显示状态</label>
										<div class="col-sm-9">
										<div class="input-medium">
						...
						<!-- 模态弹出窗内容 -->
			<form class="article" method="post" name="addarticle" enctype="multipart/form-data" action ="<%=request.getContextPath()%>/mall/updateMallProductCatSort">
			<div class="modal  fade" id="mymodal-data11" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div class="table-responsive" style="max-height: 350px;overflow: auto;">
								<table id="mallTable" class="table table-striped table-bordered table-hover" style="text-align:left;">
									<thead>
										<tr>
