				...
				<div class="goods-list order-list">
					...
					<form name="orderForm" method="post">
						...
						<c:forEach items="${temp.bookList}" var="tempCategory"> 
							...
							<div class='goods-box'>
								<div class="order-info">
									...
									<div class="goods-info">
										...
										<h3>
										<span class="goods-name">${tempCategory.bName}</span>
										<span class="goods-type2">${tempCategory.author}</span>
										</h3>
										<div class="goods-price">
											<span>￥<strong class="price-num">
											<fmt:formatNumber type="number" value="${tempCategory.price}" pattern="0.00" maxFractionDigits="2"/>  
											</strong></span>
										</div>
										</a>
								...
								<div class="control-btn clearfix">
										...
										<c:if test="${tempCategory.status ==1}">	
											<input type="button" onclick="toComment(${tempCategory.code},${temp.memberId})" value="评论" class="evaluate"/>
										</c:if>
							...
							<div class="control-btn clearfix">
								共计<font color="red"></font>本图书${temp.count} 实付:<font color="red">￥
								<fmt:formatNumber type="number" value="${temp.totalPrice}" pattern="0.00" maxFractionDigits="2"/> 
								</font> 
							</div>
							<div class="control-btn clearfix">
							...
							<c:if test="${temp.status ==1}">
								...
								<input type="button" value="取消订单" onclick="orderRefund(${temp.id},${temp.memberId})"class="confirm"/>
							...
							<c:if test="${temp.status !=1 && temp.status !=4}">
								<input type="button" value="查看物流" class="confirm"/></a>
							</c:if>	
								<c:if test="${temp.status ==2}">
								&nbsp;<input type="button" value="申请退货" onclick="fillRefund(${temp.id},${temp.memberId})"class="confirm"/>
								&nbsp;
