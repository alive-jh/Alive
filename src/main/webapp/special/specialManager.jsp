...
var topicType="${queryDto.topicType}";
...
$(document).ready( function () {
			...
			for(var i=0;i<data.infoList.length;i++){
				...
				newcontent = newcontent + '<tr><td nowrap="nowrap" align="left" >'+data.infoList[i].title+'</td>';
				...
				newcontent = newcontent + '<td nowrap="nowrap" align="left" ><nobr><button class="btn btn-xs btn-success"onclick="updateSpecialSort('+i+',0)">上移</button>&nbsp;&nbsp;&nbsp;<button onclick="updateSpecialSort('+i+',1)" class="btn btn-xs btn-danger">下移</button></nobr></td>';
...
function deleteArticle(index){
	...
	{
		document.forms['addarticle'].action ="removeSpecial?specialId="+data.infoList[index].id;
		document.forms['addarticle'].submit();
	}
}
...
function searchBook()
{
						...
						$.ajax({
							type: "POST",
							data:"name="+document.forms['addarticle'].tempName.value,
							dataType: "json",
							url: "<%=request.getContextPath() %>/special/getBook", 
							...
							success: function(data){
								...
								if(data.infoList != undefined &&data.infoList !='')
								{
									
									document.getElementById('tempDiv').innerHTML =
										'为你查询到书籍:'+data.infoList[0].name+
										'<button class="btn  btn-purple" type="button"  onclick="addmallspecifications()"><span class="icon-plus"></span>添加</button>';

									tempAuthor = data.infoList[0].author;
									tempCateId = data.infoList[0].cateId;
									tempName = data.infoList[0].name;
...
function editArticle(index)
{
	...
	else
	{
		...
		document.forms['addarticle'].sort.value = data.infoList[index].sort;	    
		$('#uTitle')[0].value = data.infoList[index].title;
		$("input[name=status]:eq("+data.infoList[index].status+")").attr("checked",'checked');
		if(data.infoList[index].logo != undefined &&data.infoList[index].logo !='')
		{
			
			document.getElementById('preview1').innerHTML = '<img width="80" height="80" src="<%=request.getContextPath() %>/wechatImages/special/'+data.infoList[index].logo+'"/>';
			document.forms['addarticle'].logo.value = data.infoList[index].logo;
			document.forms['addarticle'].oldLogo.value = data.infoList[index].logo;
			
		}
		
			if(data.infoList[index].books != undefined)
			{
				
				var tempStr = data.infoList[index].books.split(',');
				...
				for(var j=0;j<tempStr.length;j++)
				{
					 ...
					 trHtml = trHtml + '<td>'+str[0]+'</td>';
					 trHtml = trHtml + '<td>'+str[1]+'</td>';
					 trHtml = trHtml + '<td>'+str[2]+'</td>';

					 var sss = '<td><a href="javascript:delRows(@'+tempRow+'@,@'+str[0]+',@)">删除</a></td></tr>';
					sss = sss.replace(/@/g,"'");
					trHtml = trHtml+sss;
					$tr.after(trHtml);
					tempss = tempss + str[0]+",";
					...
					document.forms['addarticle'].tempIds.value = tempss;
					tempIds = tempss; 
...
function updateSpecialSort(index,type)
{
	var tempSort = "";
	
	document.forms['addarticle'].action = "./updateSpecialSort?specialId="+data.infoList[index].id+"&tempSort="+data.infoList[index].sort+"&type="+type;
	document.forms['addarticle'].submit();
}
function updateSpecialStatus(index,status)
{
	document.forms['addarticle'].action = "./updateSpecialStatus?specialId="+data.infoList[index].id+"&tempStatus="+status;
	document.forms['addarticle'].submit();
}
...
<div class="main-content">
			...
			<!-- 模态弹出窗内容 -->
			 <form class="article" method="post" name="addarticle" enctype="multipart/form-data" action ="<%=request.getContextPath()%>/special/saveSpecial">
				...
				<div class="modal-dialog" style="width:1100px;">
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">添加/修改专题信息</h4>
						</div>
						<div class="modal-body">
							...
							<div class="form-group" id="picLogo"style="height:100px;">
								...
								<div class="form-group" style="height:200px;">
										<label class="col-sm-2 control-label no-padding-right text-right">书籍列表</label>
											<div class="col-sm-10">
											<div class="input-medium">
												<div class="input-group">
												<input type="text" name="tempName">
												 <button type="button" onclick="searchBook()" class="btn btn-purple ml10 mb10">查询
										 <i class="icon-search icon-on-right bigger-110"></i>
										 </button>
