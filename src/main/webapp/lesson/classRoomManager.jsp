...
function updateUI(){
	...
	for(var i=0;i<jsonData.length;i++){
			var staStr=jsonData[i].status==1?"通过":"待审核";
			...
			newcontent = newcontent + '<td nowrap="nowrap" align="left" ><button class="btn btn-xs btn-info"  onclick="editData('+i+')" data-toggle="modal" data-target="#myModal-data"><i class="icon-edit bigger-120"></i></button></td>';
			newcontent = newcontent + '</tr>';
	}
...
$(document).ready( function () {
	jsonData=$.parseJSON($("#myJson").text());
...
function deleteData(index){
	if(confirm('你确定要删除数据吗？'))
	{
		...
		data:dataPar,
		dataType: "json",
		url: "<%=request.getContextPath() %>/api/delClassRoom", 
...
function editData(index)
{
	...
	if(index.length == '0'){
		...
		if(myData.status==1){
			$("#status").append("<option value='1' selected='selected' >通过</option>");
			$("#status").append("<option value='0' >待审核</option>");
		}else{
			$("#status").append("<option value='0' selected='selected' >待审核</option>");
			$("#status").append("<option value='1' >通过</option>");
		}
	}
}
...
function updateData(index) {
     ...
     $.ajax({
          url: '<%=request.getContextPath() %>/api/saveClassRoom' ,
          type: 'POST',
          data: formData,
          async: false,
          cache: false,
	...
	<div class="page-content">
		...
		<div class="page-header col-xs-12" style="float: left;width:100%">
			<form class="form-inline" role="form" method="post" name="searchForm"
				action="<%=request.getContextPath()%>/lesson/findClassRooms"
				...
				<div class="row">
					<div class="col-xs-12">
						<div class="table-responsive" style="text-align:right;">
							...
							<f:page page="${resultPage}" url="./findClassRooms"
		...
		<div class="modal-dialog" style="width:900px;height: 1200px;">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">添加/修改在线课堂信息</h4>
				</div>
				<div class="modal-body">

					<div id="divInput" class="form-group" style="height:35px;">
						<label class="col-sm-2 control-label no-padding-right text-right">在线课堂审核状态</label>
						<div class="col-sm-9">
