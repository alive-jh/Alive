    ...
    for (var i=0;i<audioList.length;i++) {
        ...
        (function(i){
            /*audio.addEventListener("canplay", function () {//监听audio是否加载完毕，如果加载完毕，则读取audio播放时间
                console.log("b:"+i)
                $('#audio-length-total'+audioList[i]).html(transTime(audio.duration));
            });*/
            ...
            $('#audioPlayer'+audioList[i]).click(function () {
                ...
                if (audio==document.getElementById('myAudio' + (this.id).substring(11))){
                    ...
                    //$('#audioPlayer'+(this.id).substring(11)).attr('src', '../images/audio-play.png');
                    this.src='../images/audio-play.png';
                }else {
                    console.log("bbb");
                    audio.pause();
                    audio.load();
                    $('.audioPlayer').attr('src', '../images/audio-play.png');
                }
                ...
                if (audio.paused) {
                    // 开始播放当前点击的音频
                    audio.play();
                    $('#audioPlayer'+audioList[i]).attr('src', '../images/audio-pause.png');
                } else {
                    audio.pause();
                    $('#audioPlayer'+audioList[i]).attr('src', '../images/audio-play.png');
                }
...
function audioEnded(audioIndex) {
    ...
    $('#audioPlayer'+audioIndex).attr('src', '../images/audio-play.png');
}
