...
$(document.getElementById("randcomment")).infinite().on("resize scroll", function () {
...
function getComment(momentId) {
    $.ajax({
        url: "/snp/v2/student/grade/getRandComment",
        data: {
            "momentId": momentId,
            "pageNumber": Number,
            "pageSize": 3,
...
function query() {
    ...
    for (var i = 0; i < commentList.length; i++) {
        ...
        if (commentList[i].teacherId == teacherId && teacherId!=0) {
            displayStr = "inline";
            teacherStudentIcon = "student_span";
            teacherStudentSpan = "我的";
        }
        ...
        if (commentList[i].audioUrl) {
            html += '<div id="start' + randCommentId + '" class="images_and_name">\n         ' +
                ...
                '       <div class="audio-box" onclick="playCommentAudio(' + randCommentId + ')" style="width: ' + (20 + commentList[i].audioTime * 0.5) + '%"><img src="../images/pinglun_yinping_icon@2x.png" alt=""><span>' + commentList[i].audioTime + '\'\'</span></div>' +
                '       <audio id="commentAudio' + randCommentId + '" onended="commentAudioEnded(' + randCommentId + ')" src="' + commentList[i].audioUrl + '"></audio>' +
                '    </div>'
        } else {
            html += '<div id="start' + randCommentId + '" class="images_and_name">\n         ' +
                '       <img  class="student_avatar" src="' + teacherStudenAvatar + '">\n        ' +
...
function save(txt, momentId, studentId, teacherId) {
    ...
    txt=txt.replace(regExp,"Hello");
    txt=txt.replace(reg,"Hello");
    ...
    $.post('/snp/v2/student/grade/saveRandComment?momentId=' + momentId + '&studentId=' + studentId + "&content=" + txt + '&teacherId=' + teacherId, function (res) {
...
function remove(randCommentId) {
    ...
    function confirmDeleteComment() {
        $.post('/snp/v2/student/grade/deleteRandComment?randCommentId=' + randCommentId, function (res) {
            if (res.code == 200) {
