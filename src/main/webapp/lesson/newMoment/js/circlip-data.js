...
var myVideo;
var myAudio;
var my = sessionStorage.getItem("my") == null ? 0 : sessionStorage.getItem("my"); //所有打卡为0，我的打卡为1\
if (my == 0) {
    //“所有打卡”字体加粗
    $('#all_card_span').attr('class', 'checked_card');
    //“我的打卡”的光标隐藏
    $('#my_card_hr').css('display', 'none');
    //“所有打卡”的光标显示
    $('#all_card_hr').css('display', 'block');
    //“我的打卡”字体变暗
    $('#my_card_span').attr('class', 'un_checked_card');
} else {
    //“我的打卡”字体加粗
    $('#my_card_span').attr('class', 'checked_card');
    //“我的打卡”的光标显示
    $('#my_card_hr').css('display', 'block');
    //“所有打卡”的光标隐藏
    $('#all_card_hr').css('display', 'none');
    //“所有打卡”字体变暗
    $('#all_card_span').attr('class', 'un_checked_card');
}
...
//“所有打卡”的点击事件
$('#all_card_span').click(function () {
    if ($(this).className != 'checked_card') {
        //“所有打卡”字体加粗
        $(this).attr('class', 'checked_card');
        //“我的打卡”的光标隐藏
        $('#my_card_hr').css('display', 'none');
        //“所有打卡”的光标显示
        $('#all_card_hr').css('display', 'block');
        ...
        myList = [];
        allList = [];
        getDate(my, pageNumber, pageSize);
...
window.onload = function () {
    if (sessionStorage.getItem("scrollT") != null) {
        //let pageSize;
        getDate(my, 1, sessionStorage.getItem("pageNumber") * 5);
...
function getDate(my, pageNumber, pageSize) {
    console.log("pageNumber:" + pageNumber)
    $.ajax({
        url: "/snp/v2/student/grade/getCard",
        data: {
            "studentId": studentId,
...
function avBgError(e) {
    e.src = '../images/audio-bg-default.png'
}
...
/*
$("#all_moment").pullToRefresh().on("pull-to-refresh", function () {
        allList = [];
        myList = [];
        $('#all_moment').html("");
        pageNumber = 1;
        getDate(my,pageNumber,pageSize);
        if (loading) loading = false;
        $("#all_moment").pullToRefreshDone(); // 重置下拉刷新
});*/

function thisClassCard(classRoomId) {
    sessionStorage.setItem("my", my);
    sessionStorage.setItem("scrollT", scrollT);
    sessionStorage.setItem("pageNumber", pageNumber);
    window.location.href = "../html/thisclasscard.html?student=" + studentId + "&classRoom=" + classRoomId + "&type=" + type + "&sn=" + sn + "&teacher=" + teacherId;
    //let uri="../html/thisclasscard.html?student="+studentId+"&classRoom="+classRoomId+"&type="+type+"&sn="+sn+"&teacher="+teacherId;
    //window.history.pushState('thisclasscard',null,uri);
}

function viewDetails(e) {
    //alert($(e).data("index"));
    let index = $(e).data("index");
    //$('#paragraph'+index).css("max-height","none");
    //$('#paragraph'+index).css("height","auto");
    if ($('#paragraph' + index).attr("class") == "paragraph") {
        $('#paragraph' + index).attr("class", "paragraph2");
        //$('#fold'+index).text("点击收起")
    } else {
        $('#paragraph' + index).attr("class", "paragraph");
        //$('#fold'+index).text("点击展开")
    }
}
...
function f() {
    ...
    sessionStorage.setItem("scrollT", scrollT);
    sessionStorage.setItem("pageNumber", pageNumber);
    //window.location.href="../html/search.html?student="+studentId+"&teacher="+teacherId+"&sn="+sn+"&type="+type;
    window.location.href = "../html/searchresult.html?student=" + studentId + "&teacher=" + teacherId + "&sn=" + sn + "&type=" + type;
}
