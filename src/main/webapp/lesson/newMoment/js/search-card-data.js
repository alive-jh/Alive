...
//var searchVal = unescape(getQueryString("name"));
...
$('#autoClick').bind("autoClick", function(){
    //$('#searchInput').focus();
});
...
function playRecord(groupId,name,avatar) {
    ...
    window.location="./test.html?avatar="+uri+"&groupId="+groupId;
}
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.code == 200 && res.data.list.length > 0) {
                ...
                //$(document.body).destroyInfinite();
                $('#infinite_div').hide();
                $('#all_moment').html("<p class='not_found_hint'>没找到任何内容，请确保输入是否正确哦</p>")
                //$("body").css("background","rgba(255,255,255,1)");
            }
            loading = false;
        }
