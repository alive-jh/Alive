...
public class Authentic {
	...
	private void flush() {
		synchronized(Authentic.class){
			try{
				if(token.isExpire()){
					if(EasemobKey.APP_CLIENT_ID!=null&&EasemobKey.APP_CLIENT_SECRET!=null){
						Map<String,Object>param = new HashMap<String,Object>();
						param.put("grant_type","client_credentials");
						param.put("client_id",EasemobKey.APP_CLIENT_ID);
						param.put("client_secret",EasemobKey.APP_CLIENT_SECRET);
						TalkNode res = service.request(EasemobKey.URR_TOKEN,EasemobKey.HTTP_METHOD_POST,param,null,null);
