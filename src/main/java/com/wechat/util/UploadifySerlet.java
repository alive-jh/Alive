...
public class UploadifySerlet extends HttpServlet {
	...
	//上传文件的保存路径
	protected String configPath = "/data/wechatImages/book/mp3";

	protected String dirTemp = Keys.USER_PIC_PATH+"/temp/";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String localHost = Keys.STAT_NAME;
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (!item.isFormField()) {
					...
					try{
						File uploadedFile = new File(savePath, fileName.replaceAll(" ",""));
	                    ...
	                    //out.print("http://localhost:8080/uploadify/" + configPath + "/" + ymd + "/" + fileName);
					}catch(Exception e){
						e.printStackTrace();
					}
				}else {
